...
async function createInvoiceForOveruse(auth, resolver, propertyData, pdfFilesOrKeys, jsonFilesOrKeys) {
    ...
    try {
        // 1. Strict unit ID check - NO FALLBACK
        if (!propertyData.unitCode || propertyData.unitCode.trim() === '') {
            throw new Error(`Unit ID not found in Excel for property "${propertyData.property}" - cannot create invoice`);
        }
        
        // Resolve IDs from unit code (no fallback)
